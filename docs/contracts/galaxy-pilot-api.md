...
- `POST /signals/:signalId/listen` — creates a `PENDING` resonance (a consent request). **Optional body `{openingMessage?}`** (composing the message IS the offer). When present, `openingMessage` is bounded `1..500` (the exchange-message window) and passes the **same** contact/link/handle + crisis validator as an exchange message BEFORE any write (`CRISIS_SUPPORT` / `DISALLOWED_CONTENT`; the rejected text is never echoed or logged). Omit the field for a wordless offer (send no field, not `""`; an empty string fails the min-length bound). Stored on the resonance; surfaced to the **recipient** in `GET /resonances` `received`; seeded as the sender's round-1 message on accept. **Idempotent, and the message is write-once:** a duplicate listen (even with a *different* `openingMessage`) returns the existing resonance and does **NOT** overwrite the original message.
...
- `received` ("Waiting on you", incoming pending Listens to the caller's Signals): `{resonanceId, signalId, createdAt, alias, avatarKey, intentions, openingMessage}`. This **deliberately** carries the sender's **pseudonymous** presentation (`alias`, `avatarKey`, `intentions`) so the recipient can decide who to let in, per the approved design. `openingMessage` is the sender-authored offer message (`string`), or **`null`** for a wordless offer; it is shown **only here**, to the recipient, so they judge the offer with its words attached. The pseudonymous identity is sourced **only** from the sender's `GalaxyProfile` via an explicit-allowlist projection (never a `User` / journal / PII join); an offer whose sender has no `GalaxyProfile` is omitted. This is an additive widening of the original `{resonanceId, signalId, createdAt}` shape. Block handling is unchanged (a pending Listen only exists while the pair is unblocked, and the accept path re-checks the block at action time).
- `sent` ("Waiting on them", outgoing): `{resonanceId, type, state, signalId, exchangeId?, createdAt}` where `state` is only ever **`PENDING` or `ACCEPTED`** (with `exchangeId`). The sender's view is **unchanged** by the opening-message feature: it **never** carries `openingMessage` (the sender already wrote it; it is not echoed back, and this preserves the no-state-leakage guarantee). A recipient's decline is **never** surfaced: it renders as `PENDING` (quiet) and simply drops out of the list when the Signal expires. A declined offer's `openingMessage` dies silently with it (never delivered to the sent view, and dropped from `received` because that bucket is `PENDING`-only). This is the decline representation. The same normalization applies to **every** sender-facing resonance response, including the heart/listen POST result: a re-tapped Listen that was quietly declined returns `state: "PENDING"`, never `DECLINED`.

**`POST /resonances/:resonanceId/accept`** (recipient/author only). Opens an `ACTIVE` exchange (transaction; idempotent on the unique `resonanceId`, so an Orbit's parent exchange is created exactly once). If the accepted Listen carried an `openingMessage`, it is **seeded in the same transaction as the sender's round-1 message** (`roundIndex 1`, authored by the offer sender), so the offer's words open the consented thread and the **recipient's reply completes round 1** via the ordinary turn logic. The 3-round guided model and all bounds are untouched (a wordless offer opens an empty exchange exactly as before). The seed is write-once (create path only; never on the idempotent re-accept). Every failure mode (not the recipient, not a pending Listen, blocked, expired Signal) collapses to one `409 GALAXY_RESONANCE_NOT_ACTIONABLE` so a non-recipient can't probe. Returns `{exchangeId}`.
...
| Listen with `openingMessage` | `200`; message validated + stored on the resonance, surfaced to the recipient in `received`, seeded as the sender's round-1 message on accept |
| Crisis / contact in an `openingMessage` | `400 CRISIS_SUPPORT` / `400 DISALLOWED_CONTENT`; nothing stored, text never echoed/logged (same as an exchange message) |
| Duplicate listen with a **different** `openingMessage` | idempotent `200`; returns the existing resonance and does **NOT** overwrite the original message (write-once) |
| Declined offer's `openingMessage` | dies silently with the decline: never in the sent view, dropped from `received` (PENDING-only), never delivered anywhere else |
