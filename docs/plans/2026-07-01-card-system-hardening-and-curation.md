...
- **Build 251** (`967bd59`): Challenge card fix - tapping opens a details popup (`MUPopup`) instead of blanking; body truncated to 4 lines; the wrong generic "Dive deeper"->CRI button removed; receiver-only Accept, sender sees "Waiting for {name}". Decline intentionally NOT added (X-to-dismiss covers it, per Astro).
